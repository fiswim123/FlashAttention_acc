...
%000006     input  logic        sm_start,
%000006     output logic        sm_done,
%000000     input  logic [255:0] score,       // 16 x Q8.8
%000000     input  logic [39:0] m_old,        // Q8.32
%000000     input  logic [39:0] l_old,        // Q8.32
...
%000001     input  logic [15:0] causal_mask   // 16-bit mask (1=valid, 0=masked)
        );
...
~000018     sm_state_t state, next;
 ...
 004622         next = state;
 004622         case (state)
 004607             IDLE:      next = sm_start ? FIND_MAX : IDLE;
%000003             FIND_MAX:  next = EXP_TABLE;
%000003             EXP_TABLE: next = SUM_EXP;
%000003             SUM_EXP:   next = SCALE_ACC;
%000003             SCALE_ACC: next = DONE;
%000003             DONE:      next = IDLE;
%000000             default:   next = IDLE;
            ...
            // =========================================================================
%000000     logic signed [15:0] score_elem [0:15];
...
~000016         for (int i = 0; i < 16; i++)
 000016             score_elem[i] = score[i*16 +: 16];
            ...
            // =========================================================================
%000000     logic signed [15:0] max_score;
%000000     logic signed [39:0] m_new_comb;
        
            // Apply causal mask: masked elements get -inf (0x8000 in Q8.8)
%000000     logic signed [15:0] masked_score [0:15];
...
~000016         for (int i = 0; i < 16; i++)
 000016             masked_score[i] = causal_mask[i] ? score_elem[i] : 16'sh8000;
            ...
            // 4-level max tree
%000000     logic signed [15:0] max_lvl1 [0:7];
%000000     logic signed [15:0] max_lvl2 [0:3];
%000000     logic signed [15:0] max_lvl3 [0:1];
...
%000000     wire signed [39:0] max_score_ext = {{24{max_score[15]}}, max_score};
...
%000000     logic signed [39:0] m_old_reg;
%000000     logic signed [39:0] l_old_reg;
%000000     logic signed [15:0] max_score_reg;
%000006     logic [15:0]        causal_mask_reg;
        
 004621     always_ff @(posedge clk or negedge rst_n) begin
 000019         if (!rst_n) begin
 000019             m_new_reg     <= 40'sh80_0000_0000;  // -inf in Q8.32
 000019             m_old_reg     <= 40'h0;
 000019             l_old_reg     <= 40'h0;
 000019             max_score_reg <= 16'h0;
 000019             causal_mask_reg <= 16'h0;
~004599         end else if (state == IDLE && sm_start) begin
%000003             m_new_reg     <= m_new_comb;
%000003             m_old_reg     <= m_old;
%000003             l_old_reg     <= l_old;
%000003             max_score_reg <= max_score;
%000003             causal_mask_reg <= causal_mask;
                end
            end
            ...
            // Saturate to [0, 255]
            // =========================================================================
%000000     logic [7:0]  rom_addr [0:15];
...
%000000             else if (rom_addr[i] < 8'd192)
%000000                 rom_data[i] = 16'((int'(rom_addr[i]) - 128) * 16 + 320);
                    else
%000000                 rom_data[i] = 16'((int'(rom_addr[i]) - 192) * 64 + 1344);
                end
            end
...
%000006     logic [255:0] exp_out_reg;
 ...
 004621     always_ff @(posedge clk or negedge rst_n) begin
 000019         if (!rst_n)
 000019             exp_out_reg <= 256'h0;
~004599         else if (state == EXP_TABLE)
~000048             for (int i = 0; i < 16; i++)
 000048                 exp_out_reg[i*16 +: 16] <= rom_data[i];
            end
...
%000006     logic [39:0] sum_exp_reg;
 ...
 004621     always_ff @(posedge clk or negedge rst_n) begin
 000019         if (!rst_n)
 000019             sum_exp_reg <= 40'h0;
~004599         else if (state == SUM_EXP) begin
                    // Combinational sum tree
                    logic [39:0] sum_temp;
%000003             sum_temp = 40'h0;
~000048             for (int i = 0; i < 16; i++)
 000048                 sum_temp = sum_temp + {24'h0, exp_out_reg[i*16 +: 16]};
%000003             sum_exp_reg <= sum_temp;
                end
            end
...
%000007     logic signed [39:0] m_diff;
...
%000007     logic signed [39:0] m_diff_offset;
 ...
 004622     always_comb begin
 004622         if (corr_addr == 8'd0)
 004622             corr_raw = 16'h0001;
...
%000000     logic [47:0] corr_l_prod_next;
...
%000001         corr_l_prod_next = {32'h0, corr_raw} * {8'h0, l_old_reg};
 ...
 004621     always_ff @(posedge clk or negedge rst_n) begin
 000019         if (!rst_n) begin
 000019             correction_reg <= 16'h0;
 000019             l_new_reg      <= 40'h0;
~004599         end else if (state == SCALE_ACC) begin
%000003             correction_reg <= corr_raw;
                    // Truncate: take bits [47:8] to get Q8.32 result, then add sum_exp
%000003             l_new_reg <= corr_l_prod_next[47:8] + sum_exp_reg;
                end
            end
